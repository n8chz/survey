...
ITEM
 ...
 function disp_existing_sections() {
  $db = $this->database;
  $result = $db->query("select * from section order by id");
  while ($row = $result->fetch_assoc()) {
   $section = $row["section"];
   $section_id = $row["id"];
   $attr = "delsection[$section_id]";
   echo "<fieldset id=\"qb$section_id\">\n";
   echo "<legend>Section: <span class=\"section-label\">$section</span></legend>\n";
   echo "<label for=\"$attr\">delete this section and its propositions:</label>\n";
   echo "<input id=\"$attr\" name=\"$attr\" type=\"checkbox\" />";
   $inner_result = $db->query("select id, question from question where section = $section_id");
   while ($inner_row = $inner_result->fetch_assoc()) {
    $question_id = $inner_row["id"];
    $inner_attr = "delquestion[$question_id]";
    echo "<fieldset>\n";
    echo "<legend>Proposition: {$inner_row['question']}</legend>\n";
    echo "<label for=\"$inner_attr\">delete this proposition:</label>\n";
    echo "<input id=\"$inner_attr\" name=\"$inner_attr\" type=\"checkbox\" />\n";
    echo "</fieldset>\n";
   }
   echo "</fieldset>\n";
   echo "<button type=\"button\" class=\"new-q-old-s $section_id\">Add new proposition to section above</button>\n";
  }
 }

 function del_sections($sections) {
  $db = $this->database;
  foreach ($sections as $section) {
   $db->real_query("delete from question where section=$section");
   $db->real_query("delete from section where id=$section");
  }
 }

 function del_questions($questions) {
  $db = $this->database;
  foreach ($questions as $question) {
   $db->real_query("delete from question where id=$question");
  }
  # we want to maintain referential integrity:
  $db->real_query("delete from section where id not in (select section from question)");
 }

 function add_questions($sections) {
  $db = $this->database;
  foreach ($sections as $section=>$questions) {
   foreach ($questions as $id=>$question) {
    echo "<!-- id: $id, section: $section, question: $question -->\n";
    $db->real_query("insert into question (id, section, question) values ($id, $section, '$question')");
    echo "<!-- ".$db->error." -->\n";
   }
  }
 }
 ...
 function add_sections($sections) {
  ...
  foreach ($sections as $id=>$section) {
   $db->real_query("insert into section (id, section) values ($id, '$section')");
  }
 } 
